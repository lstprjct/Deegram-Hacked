...
from bot import aria2, download_dict_lock, download_dict, STOP_DUPLICATE, TORRENT_DIRECT_LIMIT, ZIP_UNZIP_LIMIT, LOGGER, STORAGE_THRESHOLD, BASE_URL, LEECH_LIMIT
...
def __onDownloadStarted(api, gid):
    ...
    try:
        if any([STOP_DUPLICATE, TORRENT_DIRECT_LIMIT, ZIP_UNZIP_LIMIT, STORAGE_THRESHOLD, LEECH_LIMIT]):
            download = api.get_download(gid)
            ...
            if any([ZIP_UNZIP_LIMIT, TORRENT_DIRECT_LIMIT, STORAGE_THRESHOLD, LEECH_LIMIT]):
                sleep(1)
                limit = None
